...
```
tool/
  common.py                     # 共享数据访问层（药物解析 / 流式读取 / InChIKey 桥 / API 缓存 / 计时）
  drugbank_tool.py              # DrugBank 底层库（流式 XML→JSONL+偏移索引；被多个工具复用）
  mechanism_admet/              # 机制 / ADMET 组
    admetsar_predict.py
    dti_query.py
    pathway_enrich.py
    mechanism_query.py
    drugbank_metabolism_query.py
    ddi_query.py
  ade_profile/                  # ADE 谱组
    persade_drug_profile.py
    persade_contextual_retrieval.py
```
...
# 命令行
conda run -n perstox python -m tool.mechanism_admet.dti_query warfarin
```

**药物解析**：所有工具经 `common.resolve_drug()` 接受 名称 / 商品名同义词 / DrugBank ID /
...
由 `test/test_tools.py` 实测（Warfarin 驱动，perstox 环境，单次冷启动进程；数据源均为本地，
