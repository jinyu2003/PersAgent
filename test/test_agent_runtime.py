...
from models.schemas import DrugInfo, EvidencePackage, PatientInfo
from tool import agent_runtime as kb
...
def test_brain_agent_uses_llm_drug_parser() -> None:
    ...
    agent = BrainAgent(llm_json_extractor=fake_extractor)
...
def test_brain_agent_uses_llm_patient_parser() -> None:
    def fake_extractor(kind, raw, schema):
        if kind != "patient":
            return None
        return {
...
def test_brain_agent_adds_stage1_attribution_explanation() -> None:
    captured_context = {}
...
def test_brain_agent_models_only_liver_and_heart() -> None:
    attribution_calls = []
...
def test_brain_agent_accepts_bare_llm_driver_attribution() -> None:
    def fake_extractor(kind, raw, schema):
        if kind == "attribution_narrative" and raw.get("soc") == "Hepatobiliary disorders":
            return {
                "attribution_narrative": (
...
def test_brain_agent_does_not_generate_local_narrative_without_llm() -> None:
    agent = BrainAgent(llm_json_extractor=lambda kind, raw, schema: None)
    drug = DrugInfo(name="warfarin", drugbank_id="DB00682")
...
def main() -> int:
    ...
    test_brain_agent_uses_llm_drug_parser()
    test_brain_agent_uses_llm_patient_parser()
    test_brain_agent_adds_stage1_attribution_explanation()
    test_brain_agent_models_only_liver_and_heart()
    test_brain_agent_accepts_bare_llm_driver_attribution()
    test_brain_agent_does_not_generate_local_narrative_without_llm()
