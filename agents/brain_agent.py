"""Brain Agent: supervisor, two-stage reasoning, and report synthesis."""
...
from agents.input_parser import drug_info_kwargs_from_input, normalize_drug_input
from agents.mechanism_chain_builder import MechanismChainBuilder
from models.formatting import to_plain_dict
...
class BrainAgent:
    ...
    def parse_patient_info(self, raw: Any) -> PatientInfo:
        ...
        patient = PatientInfo(**self._patient_info_payload(llm_payload or raw))
    ...
    def parse_drug_info(self, raw: Any) -> DrugInfo:
        ...
        return DrugInfo(**self._drug_info_payload(raw, deterministic_payload, llm_payload))

    def _parse_user_input_with_llm(
        self,
        *,
        kind: str,
        raw: Any,
        ...
        if self._llm_json_extractor is not None:
            return self._coerce_llm_payload(self._llm_json_extractor(kind, raw, schema))
    ...
    def _patient_info_payload(self, raw: Any) -> Dict[str, Any]:
        if isinstance(raw, Mapping):
            payload = {key: value for key, value in dict(raw).items() if key in _PATIENT_INFO_FIELDS}
        else:
            payload = {}

        payload["patient_id"] = self._string_value(payload.get("patient_id")) or "anonymous"
        payload["age"] = self._int_value(payload.get("age"), default=0)
        payload["sex"] = self._normalize_sex(payload.get("sex"))

        for field in ("weight_kg", "alt_u_l", "ast_u_l", "bilirubin_mg_dl", "creatinine_mg_dl", "egfr_ml_min"):
            payload[field] = self._float_or_none(payload.get(field))

        child_pugh = self._string_value(payload.get("child_pugh"))
        ...
        payload["hla_types"] = self._string_list(payload.get("hla_types"))
        payload["medical_history"] = self._string_list(payload.get("medical_history"))
        payload["concomitant_medications"] = self._string_list(payload.get("concomitant_medications"))
        payload["organ_function"] = (
            dict(payload.get("organ_function") or {}) if isinstance(payload.get("organ_function"), Mapping) else {}
        )
        payload["exposure"] = dict(payload.get("exposure") or {}) if isinstance(payload.get("exposure"), Mapping) else {}
        payload["pregnancy_status"] = self._normalize_pregnancy_status(payload.get("pregnancy_status"))
        missing = set(self._string_list(payload.get("missing_modalities")))
        if payload["age"] == 0:
            missing.add("age")
        payload["missing_modalities"] = sorted(missing)
        return payload

    def _drug_info_payload(
        self,
        raw: Any,
        deterministic_payload: Dict[str, Any],
        llm_payload: Optional[Dict[str, Any]],
    ) -> Dict[str, Any]:
        payload = {key: deterministic_payload.get(key) for key in _DRUG_INFO_FIELDS}
        llm_payload = llm_payload or {}
        if "drug_name" in llm_payload and "name" not in llm_payload:
            llm_payload["name"] = llm_payload["drug_name"]

        for key in _DRUG_INFO_FIELDS:
            value = llm_payload.get(key)
            if self._has_llm_value(value):
                payload[key] = value
        ...
        normalized = normalize_drug_input(resolver_payload if any(resolver_payload.values()) else raw)
        payload["name"] = normalized["drug_name"] or self._string_value(payload.get("name")) or "unknown"
        payload["drugbank_id"] = normalized["drugbank_id"] or self._string_value(payload.get("drugbank_id"))
        payload["inchi_key"] = normalized["inchi_key"] or self._string_value(payload.get("inchi_key"))
        payload["smiles"] = normalized["smiles"] or self._string_value(payload.get("smiles"))
        payload["dose"] = self._string_value(payload.get("dose")) or "unspecified"
        payload["route"] = self._string_value(payload.get("route")) or "unspecified"
        payload["frequency"] = self._string_value(payload.get("frequency"))
        payload["form"] = self._string_value(payload.get("form"))
        payload["target_description"] = self._string_value(payload.get("target_description"))
        payload["known_toxicities"] = self._string_list(payload.get("known_toxicities"))
        return payload

    @staticmethod
    def _has_llm_value(value: Any) -> bool:
        if value is None:
            return False
        if isinstance(value, str):
            return value.strip().lower() not in _EMPTY_LLM_VALUES
    ...
    @staticmethod
    def _normalize_pregnancy_status(value: Any) -> Optional[str]:
        text = (str(value).strip().lower() if value is not None else "")
    ...
    def merge_evidence_packages(
        ...
        """Brain-owned accumulation of evidence across serial retrieval stages."""
        if existing is None:
            return new_package
    ...
    def build_universal_report(
        ...
        for organ in ORGAN_SYSTEMS:
            ...
            mechanism_chains = self.mechanism_chain_builder.build_for_organ(
                drug_info=drug_info,
                tool_results=tool_results,
                organ=organ,
                soc=soc,
            ...
            if local_summary:
                ...
                mechanism_text = (
                    f"{self.mechanism_chain_builder.summarize(mechanism_chains)} "
    ...
    def build_personalized_report(
        ...
        factors = self._collect_patient_attributions(patient_info, drug_info, evidence_package)
    ...
    def _collect_patient_attributions(
        ...
        if patient_info.child_pugh in {"B", "C"}:
            hepatic_magnitude = 1.45 if patient_info.child_pugh == "B" else 1.80
            heme_magnitude = 1.25 if patient_info.child_pugh == "B" else 1.45
            self._add_factor(
                factors,
                "liver",
                "organ_function",
                f"Child-Pugh {patient_info.child_pugh} hepatic impairment",
                "up",
                hepatic_magnitude,
                f"ORG-HEPATIC-CHILD-PUGH-{patient_info.child_pugh}",
                "Clinical rule",
                4,
            ...
            self._add_factor(
                ...
                "hematologic",
                "comorbidity",
                f"Child-Pugh {patient_info.child_pugh} cirrhosis",
                "up",
                heme_magnitude,
                f"COMORB-CIRRHOSIS-CHILD-PUGH-{patient_info.child_pugh}",
                "Clinical rule",
                4,
                "Cirrhosis can destabilize coagulation and anticoagulant response.",
            )

        if patient_info.egfr_ml_min is not None and patient_info.egfr_ml_min < 60:
            magnitude = 1.30 if patient_info.egfr_ml_min >= 30 else 1.60
            self._add_factor(
                factors,
                "kidney",
                "organ_function",
                f"eGFR {patient_info.egfr_ml_min:g} mL/min",
                "up",
                magnitude,
                "ORG-EGFR-LT60",
                "Clinical rule",
            ...
            if drug_lower in {"warfarin"}:
                self._add_factor(
                    ...
                    "hematologic",
                    "organ_function",
                    "Moderate renal impairment during anticoagulation",
                    "up",
                    1.10,
                    "ORG-EGFR-WARFARIN-BLEEDING",
                    "Clinical rule",
                    4,
    ...
    def _target_pathway_for_organ(
        ...
        if mechanism_chains:
            ...
            target_pathway.append(
                {
                    "source": "MechanismChainBuilder fallback",
